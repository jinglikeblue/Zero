...
namespace IL.Demo
{
    class HelpWin : AView
    {
        ...
        {
            UIEventListener.Get(GO).onClick -= ShowNextPage;
        ...
        {
            _tip = GO.GetComponent<Image>();
            var resData = GO.GetComponent<ObjectBindingData>();
            var list = resData.Find("tips").Value.list;
            _tipSpriteList = list;
            var stringData = GO.GetComponent<StringBindingData>();
